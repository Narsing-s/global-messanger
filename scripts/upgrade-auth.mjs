...
if (!advanced.includes("'/api/auth/register-email'")) {
  ...
  app.post('/api/auth/register-email', async (request, reply) => {
    const parsed = z.object({ username: z.string().trim().min(3).max(24).regex(/^[a-zA-Z0-9_]+$/), displayName: z.string().trim().min(1).max(60), email: z.string().trim().email().max(320), password: z.string().min(8).max(128) }).safeParse(request.body ?? {});
