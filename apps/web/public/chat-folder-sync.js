(() => {
  ...
  const idMap = () => read('gm_chat_conversation_ids', {});
  const remember = (label, id) => { if (!label || !id) return; const map = idMap(); map[label] = String(id); write('gm_chat_conversation_ids', map); };
  ...
  async function syncIds() {
    ...
    try {
      ...
      [...list.querySelectorAll('.chat-item')].forEach(item => {
        const label = item.querySelector('.chat-copy b')?.textContent?.trim(); if (!label) return;
        const c = findByTitle(rows, label); const id = c?.id || map[label];
        if (id) { item.setAttribute('data-gm-conversation-id', String(id)); remember(label, id); }
      });
  ...
  function ensureContactFolderRow() {
    ...
    const row = document.createElement('div'); row.className = 'gm-ci-row'; row.dataset.act = 'archive';
  ...
  function folderKeys() { return { favorites: read('gm_chat_favorites', []), archived: read('gm_chat_archived', []) }; }

  async function handleFolderAction(row) {
    const action = row?.dataset?.act; if (action !== 'favorite' && action !== 'archive') return false;
    const label = title(); if (!label) return true;
    const rows = await conversations().catch(() => []);
    const c = findByTitle(rows, label); const rememberedId = idMap()[label]; const id = String(c?.id || rememberedId || '');
    if (!id) { alert('Conversation not found.'); return true; }
    remember(label, id);
    const keys = folderKeys();
    if (action === 'favorite') {
      const index = keys.favorites.indexOf(id);
      if (index >= 0) { keys.favorites.splice(index, 1); write('gm_chat_favorites', keys.favorites); alert(`${label} removed from favourites.`); }
      else { keys.favorites.push(id); write('gm_chat_favorites', keys.favorites); alert(`${label} added to favourites. Open Favorites to see this chat.`); }
      const p = read('gm_contact_preferences', {}); p[label] = {...(p[label] || {}), favorite: keys.favorites.includes(id)}; write('gm_contact_preferences', p);
      window.dispatchEvent(new CustomEvent('gm:chat-folders-refresh')); document.getElementById('gm-contact-info-final')?.remove(); return true;
    }
    try {
      const archived = keys.archived.includes(id);
      if (archived) { await request(`/api/conversations/${encodeURIComponent(id)}/restore`, {method:'POST'}); write('gm_chat_archived', keys.archived.filter(x => x !== id)); alert(`${label} restored to Chats.`); }
      else { await request(`/api/conversations/${encodeURIComponent(id)}`, {method:'DELETE'}); write('gm_chat_archived', [...keys.archived, id]); alert(`${label} moved to Archive. Open Archive to see this chat.`); }
      const p = read('gm_contact_preferences', {}); p[label] = {...(p[label] || {}), archived: !archived}; write('gm_contact_preferences', p);
      window.dispatchEvent(new CustomEvent('gm:chat-folders-refresh')); document.getElementById('gm-contact-info-final')?.remove();
  ...
  syncIds(); setInterval(syncIds, 1500); setInterval(ensureContactFolderRow, 300);
})();
