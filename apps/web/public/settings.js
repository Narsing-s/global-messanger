(() => {
  ...
  function addButton(){
    ...
    const b=document.createElement('button'); b.id='gm-settings-button'; b.title='Settings'; b.setAttribute('aria-label','Settings'); b.textContent='⚙️';
  ...
  async function openSettings(){
    ...
    overlay.innerHTML=`<div class="gm-settings-card"><div class="gm-settings-head"><div><h2>Settings</h2><p>Control your Global Messenger account, privacy and app experience.</p></div><button class="gm-settings-close" aria-label="Close">×</button></div>
      <section class="gm-settings-section"><h3>Account</h3><p>Your public profile details.</p><div class="gm-setting-row"><div><b>Display name</b><span>This is the name other people see.</span></div><input id="gm-display-name" type="text" maxlength="80"></div><div style="display:flex;justify-content:flex-end"><button class="gm-save" id="gm-save-profile">Save profile</button></div><div id="gm-profile-status"></div></section>
      <section class="gm-settings-section"><h3>Appearance</h3><p>These preferences are saved on this device.</p><div class="gm-setting-row"><div><b>Theme</b><span>Choose light, dark, or follow your device.</span></div><select id="gm-theme"><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></div><div class="gm-setting-row"><div><b>Compact chats</b><span>Reduce spacing to show more messages and conversations.</span></div><button class="gm-toggle ${p.compact?'on':''}" id="gm-compact"><i></i></button></div><div class="gm-setting-row"><div><b>Reduce motion</b><span>Minimize UI animations for accessibility.</span></div><button class="gm-toggle ${p.reducedMotion?'on':''}" id="gm-motion"><i></i></button></div></section>
      <section class="gm-settings-section"><h3>Privacy</h3><p>Server-backed privacy controls apply to your account.</p><div class="gm-setting-row"><div><b>Last seen</b><span>Who can see when you were last active.</span></div><select id="gm-last-seen"><option value="everyone">Everyone</option><option value="contacts">Contacts</option><option value="nobody">Nobody</option></select></div><div class="gm-setting-row"><div><b>Profile photo</b><span>Who can see your profile photo.</span></div><select id="gm-profile-photo"><option value="everyone">Everyone</option><option value="contacts">Contacts</option><option value="nobody">Nobody</option></select></div><div style="display:flex;justify-content:flex-end"><button class="gm-save" id="gm-save-privacy">Save privacy</button></div><div id="gm-privacy-status"></div></section>
      <section class="gm-settings-section"><h3>Security</h3><p>Review active sessions and revoke access you no longer recognize.</p><div id="gm-sessions"><div class="gm-note">Loading sessions…</div></div><div style="display:flex;justify-content:flex-end;margin-top:12px"><button class="gm-danger" id="gm-revoke-all">Sign out other devices</button></div></section>
      <section class="gm-settings-section"><h3>About</h3><div class="gm-note">Global Messenger • Settings are functional and saved locally or securely on your account depending on the option.</div></section>
    </div>`;
    document.body.appendChild(overlay);
    overlay.querySelector('.gm-settings-close').onclick=()=>overlay.remove();
    overlay.onmousedown=e=>{if(e.target===overlay)overlay.remove()};
    ...
    try{
      ...
      sessions.innerHTML=(Array.isArray(list)&&list.length)?list.map(s=>`<div class="gm-session"><div><b>${String(s.deviceName||s.platform||'Device').replace(/[<>]/g,'')}</b><small>${String(s.platform||'web')} • Last active ${s.lastSeenAt?new Date(s.lastSeenAt).toLocaleString():'unknown'}</small></div><button class="gm-danger" data-session="${s.id}">Revoke</button></div>`).join(''):'<div class="gm-note">No tracked sessions yet. New sessions are registered when supported by the server.</div>';
      sessions.querySelectorAll('[data-session]').forEach(btn=>btn.onclick=async()=>{try{await fetch(API+'/api/sessions/'+encodeURIComponent(btn.dataset.session),{method:'DELETE',headers:{Authorization:`Bearer ${token()}`}});btn.closest('.gm-session')?.remove();}catch(e){alert(e.message||'Unable to revoke session')}});
    }catch(e){sessions.innerHTML=`<div class="gm-note">Unable to load sessions: ${String(e.message||e)}</div>`}
    overlay.querySelector('#gm-revoke-all').onclick=async()=>{if(!confirm('Sign out all other active devices?'))return;try{await fetch(API+'/api/sessions',{method:'DELETE',headers:{Authorization:`Bearer ${token()}`}});alert('Other devices have been signed out.');}catch(e){alert(e.message||'Unable to revoke sessions')}};
  }
