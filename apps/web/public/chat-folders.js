(() => {
  ...
  const refresh = () => {
    ...
    const archived = read('gm_chat_archived', []), favorites = read('gm_chat_favorites', []), pinned = read('gm_chat_pinned', []);
    ...
    [...list.querySelectorAll('.chat-item')].forEach(item => {
      ...
      const isArchived = archived.includes(id), isFavorite = favorites.includes(id), pinIndex = pinned.indexOf(id);
  ...
  document.addEventListener('click', async e => {
    ...
    e.preventDefault(); e.stopImmediatePropagation(); const c = await activeConversation().catch(() => null); if (!c) return alert('Open a conversation first.'); const id = String(c.id);
