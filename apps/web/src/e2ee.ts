...
async function deriveAesKey(privateJwk: JsonWebKey, publicJwk: JsonWebKey, conversationId: string, salt?: Uint8Array) {
  ...
  const pending = (async () => {
    ...
    const infoBuffer = enc.encode('gm-e2ee-v1').slice().buffer as ArrayBuffer;
    return crypto.subtle.deriveKey({ name: 'HKDF', hash: 'SHA-256', salt: saltBuffer, info: infoBuffer }, hkdf, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
  })();
