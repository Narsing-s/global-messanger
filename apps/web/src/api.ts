...
export const api = {
  ...
  direct: (userId: string) =>
    request('/api/conversations/direct', {
      method: 'POST',
  ...
  messages: (id: string, limit = 100) =>
    request(`/api/conversations/${id}/messages?limit=${limit}`),
  read: (id: string) =>
  ...
  upload: (file: File) => {
    ...
    f.append('file', file);
    return request('/api/uploads', { method: 'POST', body: f });
  },
  react: (id: string, emoji: string) =>
    request(`/api/messages/${id}/reactions`, {
      method: 'POST',
