...
function normalizeConversations(value: any): ConversationResponse[] { const list = Array.isArray(value) ? value : value?.conversations; if (!Array.isArray(list)) return []; const seenPairs = new Set<string>(), seenIds = new Set<string>(); const result: ConversationResponse[] = []; for (const raw of list) { const conversation = normalizeConversation(raw); if (!conversation.id || seenIds.has(conversation.id)) continue; seenIds.add(conversation.id); const pair = directPairKey(conversation); if (pair) { if (seenPairs.has(pair)) continue; seenPairs.add(pair); } result.push(conversation); } return result; }
