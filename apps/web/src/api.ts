...
export const api = {
  ...
  direct: async (userId: string) => {
    ...
    finally { if (directRequests.get(key) === promise) directRequests.delete(key); }
  },
  ...
  pin: (conversationId: string, messageId: string) => request(`/api/conversations/${encodeURIComponent(conversationId)}/pins`, {
    method: 'POST', body: JSON.stringify({ messageId })
  }),
  unpin: (conversationId: string, messageId: string) => request(`/api/conversations/${encodeURIComponent(conversationId)}/pins/${encodeURIComponent(messageId)}`, {
    method: 'DELETE'
  }),
  searchMessages: async (q: string, conversationId?: string) => request(
    `/api/messages/search?q=${encodeURIComponent(q)}${conversationId ? `&conversationId=${encodeURIComponent(conversationId)}` : ''}`
  ),
  profile: () => request('/api/profile/me'),
  updateProfile: (data: { displayName?: string; avatarUrl?: string | null }) => request('/api/profile/me', {
    method: 'PATCH', body: JSON.stringify(data)
  }),
  renameGroup: (id: string, title: string) => request(`/api/conversations/${encodeURIComponent(id)}/group`, {
    method: 'PATCH', body: JSON.stringify({ title })
  }),
  addGroupMember: (id: string, userId: string) => request(`/api/conversations/${encodeURIComponent(id)}/members`, {
    method: 'POST', body: JSON.stringify({ userId })
  }),
  removeGroupMember: (id: string, userId: string) => request(`/api/conversations/${encodeURIComponent(id)}/members/${encodeURIComponent(userId)}`, {
    method: 'DELETE'
  }),
  forwardMessage: (messageId: string, conversationId: string) => request('/api/messages/forward', {
    method: 'POST', body: JSON.stringify({ messageId, conversationId })
  }),
  editMessage: (id: string, body: string) => request(`/api/messages/${encodeURIComponent(id)}`, {
  ...
  unreact: (id: string, emoji: string) => request(`/api/messages/${encodeURIComponent(id)}/reactions`, {
  ...
  registerDevice: (token: string, platform: string) => request('/api/devices', {
    method: 'POST',
    body: JSON.stringify({ token, platform })
  }),
  aiAssist: (prompt: string, context?: string) => request('/api/ai/assist', {
    method: 'POST',
