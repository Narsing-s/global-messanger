...
// then same-origin web hosting, then the known production API. A successful origin is persisted
// so every subsequent request on that device uses the working server.
...
// Put the two major product surfaces inside the authenticated workspace, not on the login page.
...
const oldQuick = '<div className="quick-actions"><span>Quick Actions</span>';
const newQuick = '<div className="quick-actions"><span>Quick Actions</span><button type="button" onClick={()=>window.__gmProductCenter?.open?.()||window.__gmAdvancedCompletion?.open?.()}><Sparkles/>Product Center</button><button type="button" onClick={()=>window.dispatchEvent(new CustomEvent(\'gm:options\'))}><Menu/>Messenger Tools</button>';
if (!source.includes('quick-actions') || !source.includes('Product Center</button><button type="button" onClick={()=>window.dispatchEvent(new CustomEvent(\'gm:options\'))}')) source = source.replace(oldQuick, newQuick);
