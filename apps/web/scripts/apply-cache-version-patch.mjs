...
for (const script of ['apply-production-feature-activation.mjs','apply-chat-workspace-patch.mjs']) {
...
const stamp = `release-${new Date().toISOString().slice(0,10).replaceAll('-','')}-v16`;
...
for (const relative of ['index.html','public/sw.js']) {
  ...
  source = source.replace(/global-messenger-shell-v\d+/g, 'global-messenger-shell-v16');
