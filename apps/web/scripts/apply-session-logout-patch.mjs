...
if (!source.includes(guard)) {
  ...
  const next = `  async function logout(){${guard} const token=localStorage.getItem('gm_token'); try { if(token) await fetch(\`${'${API}'}/api/auth/logout\`,{method:'POST',headers:{Authorization:\`Bearer ${'${token}'}\`}}); } catch {} localStorage.clear();socket?.disconnect();setUser(null);setChats([]);setActive(null)}`;
