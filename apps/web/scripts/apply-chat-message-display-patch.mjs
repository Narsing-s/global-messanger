...
function installMessageQuickActions(){
  if((window as any).__gmMessageQuickActions)return;
  (window as any).__gmMessageQuickActions=true;
  let lastId=''; let lastAt=0;
  document.addEventListener('click',(event:any)=>{
    const target=event.target as HTMLElement|null;
    if(target?.closest?.('.bubble-menu,.bubble-actions,.emoji-panel,button,input,textarea'))return;
    const row=target?.closest?.('.bubble-row') as HTMLElement|null;
    if(!row)return;
    const id=row.getAttribute('data-message-id')||'';
    const now=Date.now();
    if(id&&id===lastId&&now-lastAt<420){
      const menuButton=row.querySelector('.bubble-menu') as HTMLButtonElement|null;
      menuButton?.click();
      window.setTimeout(()=>{
        const action=[...row.querySelectorAll('button')].find((b:any)=>/react/i.test((b.textContent||'').trim())) as HTMLButtonElement|null;
        action?.click();
      },40);
      lastId=''; lastAt=0;
      return;
...
`;
  ...
  source = source.replace("{reply.body}</span>","{messagePreview(reply)}</span>");
  source = source.replace(
    "function App(){",
    "function App(){installMessageQuickActions();"
  );
  fs.writeFileSync(file, source);
  console.log('Chat message display patch v3 applied');
}
