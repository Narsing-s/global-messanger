...
if (!source.includes('gm-call-behavior-v2')) {
  ...
  source = source.replace(
    ...
    "renderCall(`Calling ${chatLabel()}`, kind === 'video'); hideSignalMessages(); if (isCallPeerOnline(c)) startRingtone(kind === 'video');"
  );
  source = source.replace(
    "async function answer(d: any) { if (pc) { await pc.setRemoteDescription(d.answer);",
    "async function answer(d: any) { if (pc) { stopRingtone(); setConnectedTitle(`Connected with ${chatLabel()}`); startTimer(); await pc.setRemoteDescription(d.answer);"
  );
  source = source.replace(
