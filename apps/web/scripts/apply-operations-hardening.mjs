...
["request('/api/reports',{method:'POST',body:JSON.stringify({targetId:id})})","request('/api/trust/reports',{method:'POST',body:JSON.stringify({targetId:id,reason:'user report'})})"],
["setStatus('Rate limiting and abuse protection are server-side controls.')","run(()=>request('/api/trust/status'),'Safety status loaded')"]
];
