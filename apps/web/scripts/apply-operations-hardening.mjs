...
["setStatus('Open the conversation call controls for voice/video.')","document.querySelector<HTMLButtonElement>('[title=\"Voice call\"]')?.click()"],
["setStatus('Open the conversation call controls for video.')","document.querySelector<HTMLButtonElement>('[title=\"Video call\"]')?.click()"],
["setStatus('Screen sharing is available from the active call.')","document.querySelector<HTMLButtonElement>('[title*=\"Screen\"],button[data-gm-action=\"screen-share\"]')?.click()||setStatus('Start a video call first, then use Screen share from the call controls.')"],
