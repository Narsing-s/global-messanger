...
if (!source.includes("s.on('message:delivered',(d:any)=>")) {
  source = source.replace("s.on('message:delivered',()=>setSocketError(''));", "s.on('message:delivered',(d:any)=>{if(d?.clientId)removeOfflineMessage(String(d.clientId));setSocketError('')});");
}
