...
export async function sendWelcomeEmail(to: string, displayName: string) {
  const config = smtpConfig();
  const safeName = htmlEscape(displayName || 'there');
  const appUrl = String(process.env.APP_WEB_ORIGIN || process.env.PASSWORD_RESET_WEB_ORIGIN || '').trim().replace(/\/$/, '');
  const launchButton = appUrl
    ? `<p><a href="${htmlEscape(appUrl)}" style="display:inline-block;background:#2563eb;color:#fff;text-decoration:none;padding:12px 20px;border-radius:10px;font-weight:600">Open Global Messenger</a></p>`
    : '';
  const html = `<!doctype html><html><body style="font-family:Arial,sans-serif;line-height:1.6;color:#172033;background:#f6f7fb;padding:32px"><div style="max-width:600px;margin:0 auto;background:#fff;padding:36px;border:1px solid #e5e7eb;border-radius:18px"><div style="font-size:28px;font-weight:700;margin-bottom:8px">🌍 Global Messenger</div><h1 style="margin-bottom:8px">Welcome, ${safeName}! 🎉</h1><p>Thank you for creating your Global Messenger account. We’re excited to have you with us.</p><h2>What you can do</h2><ul><li>💬 Chat and stay connected with people.</li><li>👥 Find users and start direct conversations.</li><li>📎 Share messages and supported attachments.</li><li>🔖 Bookmark and pin important messages.</li><li>🤖 Use Global Messenger AI when it is enabled for your account.</li><li>🔐 Manage your account securely, including password reset.</li></ul><h2>Getting started</h2><p>Search for a person, open a conversation, and start messaging. Keep your account credentials private and use a strong password.</p>${launchButton}<p>Thanks for choosing Global Messenger. We hope you enjoy the experience!</p><p style="font-size:12px;color:#6b7280">This is a welcome message from Global Messenger. You can ignore this email if you did not create an account.</p></div></body></html>`;
