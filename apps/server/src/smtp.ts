...
export async function sendPasswordResetEmail(to: string, displayName: string, resetUrl: string) {
  ...
  // The reset URL is already generated by the server for the correct web app.
  // Do not rewrite its path: /reset-password.html is intentionally handled by Vite/Render.
  const normalizedResetUrl = resetUrl;
