...
async function requireSupportTeam(
  app: FastifyInstance,
  prisma: PrismaClient,
  request: any,
  reply: any,
...
export async function registerEmailAuthRoutes(app: FastifyInstance, prisma: PrismaClient) {
  app.post('/api/auth/register-email', async (request, reply) => {
    ...
    if (!parsed.success) return reply.badRequest('Please check all registration fields.');
    ...
    const user = await prisma.user.create({
      data: { username, displayName: parsed.data.displayName, email, phoneNumber, passwordHash },
    });
    ...
    try {
      ...
      app.log.error(error, 'Welcome email failed after account creation');
    }
    ...
    await prisma.loginHistory.create({
      data: {
        ...
        ipAddress: String(request.ip || '').slice(0, 64),
      },
    });
  ...
  app.post('/api/auth/login-email', async (request, reply) => {
    ...
    const user = await prisma.user.findFirst({
      where: { OR: [{ email: identifier }, { username: identifier }, { phoneNumber: phone }] },
    });
    ...
    await prisma.loginHistory.create({
      data: {
        userId: user.id,
        method: user.totpEnabled ? 'password+2fa-pending' : 'password',
        success: true,
        platform: 'mobile-or-web',
        userAgent: String(request.headers['user-agent'] || '').slice(0, 500),
        ipAddress: String(request.ip || '').slice(0, 64),
  ...
  app.post('/api/support/requests', async (request, reply) => {
    const parsed = z.object({
      requestId: supportRequestId.optional(),
    ...
    if (await prisma.supportRequest.findUnique({ where: { requestId }, select: { id: true } })) {
      requestId = `GM-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
    }
      ...
      include: {
        resolution: {
          select: {
            ...
            resolutionTimeSecs: true,
            closedAt: true,
            resolvedBy: { select: { displayName: true, username: true } },
          },
        },
      },
    });
    ...
    return {
      ...
      resolution: item.resolution
        ? {
            ...
            resolutionNotes: item.resolution.resolutionNotes,
            resolutionTimeSecs: item.resolution.resolutionTimeSecs,
            closedAt: item.resolution.closedAt,
            resolvedBy: item.resolution.resolvedBy?.displayName || item.resolution.resolvedBy?.username || null,
          }
        : null,
    };
  });
  ...
  app.get('/api/support/team/requests', { preHandler: [app.authenticate] }, async (request, reply) => {
    const agent = await requireSupportTeam(app, prisma, request, reply);
    if (!agent) return;
  ...
  app.post('/api/support/team/requests/:requestId/close', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    const result = await prisma.$transaction(async tx => {
      const resolution = await tx.supportResolution.create({
        data: {
    ...
    return {
      ok: true,
      requestId: result.closed.requestId,
      status: result.closed.status,
      closedAt: result.resolution.closedAt,
      resolutionType: result.resolution.resolutionType,
      resolutionTimeSecs: result.resolution.resolutionTimeSecs,
      resolutionTimeMinutes: Math.round((result.resolution.resolutionTimeSecs / 60) * 100) / 100,
      resolvedBy: agent.displayName || agent.username,
    };
  });
  ...
  app.post('/api/conversations/:id/report', { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    await sendReportEmail({
      reporterEmail: me?.email || undefined,
      reporterName: me?.displayName || me?.username,
      reportedName: reported?.displayName || reported?.username,
      conversationId,
