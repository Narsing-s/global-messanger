...
const WEB_ORIGIN = process.env.WEB_ORIGIN ?? 'http://localhost:5173,https://web.narsingbeesetti006.workers.dev,https://global-messanger.onrender.com';
...
await app.register(staticPlugin, { root: path.join(__dirname, '../uploads'), prefix: '/uploads/' });
app.get('/', async () => ({ ok: true, service: 'global-messenger-api', health: '/health' }));
...
const io = new SocketIOServer(httpServer, { cors: { origin: (origin, cb) => cb(null, isAllowedOrigin(origin)), credentials: true } });
io.on('connection', socket => {
  ...
  socket.on('send_message', async (payload: { conversationId: string; senderId: string; body: string }) => {
    const message = await prisma.message.create({ data: { conversationId: payload.conversationId, senderId: payload.senderId, body: payload.body } });
