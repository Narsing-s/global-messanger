...
async function assertRoomMember(prisma: PrismaClient, roomId: string, userId: string) {
  const membership = await prisma.conversationMember.findFirst({ where: { conversationId: roomId, userId }, select: { userId: true } });
  if (!membership) throw new Error('You are not a member of this call room');
}

export async function registerSfu(app: FastifyInstance, prisma: PrismaClient) {
  const auth = { preHandler: [app.authenticate] };

  app.get('/api/sfu/:roomId/capabilities', auth, async request => {
    const roomId = String((request.params as any).roomId);
    const userId = (request.user as AuthUser).id;
    try { await assertRoomMember(prisma, roomId, userId); } catch { throw app.httpErrors.forbidden('Call room access denied'); }
    return { routerRtpCapabilities: (await getRouter(roomId)).rtpCapabilities };
  });

  app.post('/api/sfu/:roomId/transport', auth, async request => {
    const roomId = String((request.params as any).roomId);
    const userId = (request.user as AuthUser).id;
    try { await assertRoomMember(prisma, roomId, userId); } catch { throw app.httpErrors.forbidden('Call room access denied'); }
    ...
    transports.set(transport.id, transport); transportRooms.set(transport.id, roomId); transportUsers.set(transport.id, userId);
    const cleanup = () => { transports.delete(transport.id); transportRooms.delete(transport.id); transportUsers.delete(transport.id); };
  ...
  app.post('/api/sfu/transport/:id/connect', auth, async request => {
    const id = String((request.params as any).id);
    const userId = (request.user as AuthUser).id;
    const transport = transports.get(id);
    if (!transport) throw app.httpErrors.notFound('SFU transport not found');
    if (transportUsers.get(id) !== userId) throw app.httpErrors.forbidden('Transport ownership mismatch');
    await transport.connect({ dtlsParameters: (request.body as any)?.dtlsParameters });
    return { ok: true };
  });

  app.post('/api/sfu/transport/:id/produce', auth, async request => {
    ...
    if (transportUsers.get(transportId) !== userId) throw app.httpErrors.forbidden('Transport ownership mismatch');
    ...
    producers.set(producer.id, producer); producerRooms.set(producer.id, roomId); producerUsers.set(producer.id, userId);
    const cleanup = () => { producers.delete(producer.id); producerRooms.delete(producer.id); producerUsers.delete(producer.id); };
    producer.on('transportclose', cleanup); producer.on('close', cleanup);
    return { id: producer.id };
  });

  app.get('/api/sfu/:roomId/producers', auth, async request => {
    const roomId = String((request.params as any).roomId);
    const userId = (request.user as AuthUser).id;
    try { await assertRoomMember(prisma, roomId, userId); } catch { throw app.httpErrors.forbidden('Call room access denied'); }
  ...
  app.post('/api/sfu/transport/:id/consume', auth, async request => {
    const transportId = String((request.params as any).id);
    const userId = (request.user as AuthUser).id;
    const transport = transports.get(transportId); const roomId = transportRooms.get(transportId);
    if (!transport || !roomId) throw app.httpErrors.notFound('SFU transport not found');
    ...
    consumers.set(consumer.id, consumer); consumerUsers.set(consumer.id, userId);
    const cleanup = () => { consumers.delete(consumer.id); consumerUsers.delete(consumer.id); };
  ...
  app.post('/api/sfu/consumer/:id/resume', auth, async request => {
    const id = String((request.params as any).id);
    const userId = (request.user as AuthUser).id;
    const c = consumers.get(id);
    if (!c) throw app.httpErrors.notFound('Consumer not found');
    if (consumerUsers.get(id) !== userId) throw app.httpErrors.forbidden('Consumer ownership mismatch');
    await c.resume();
    return { ok: true };
  });

  app.get('/api/sfu/health', async () => ({ ok: Boolean(worker && !worker.closed), workers: worker && !worker.closed ? 1 : 0, rooms: routers.size, transports: transports.size, producers: producers.size, consumers: consumers.size, rtcPortRange: `${process.env.MEDIASOUP_RTC_MIN_PORT ?? 40000}-${process.env.MEDIASOUP_RTC_MAX_PORT ?? 49999}` }));
}
