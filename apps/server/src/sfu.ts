...
async function getRouter(roomId: string) {
  ...
  const router = await w.createRouter({ mediaCodecs: codecs });
  routers.set(roomId, router);
  return router;
}

export async function registerSfu(app: FastifyInstance, _prisma: PrismaClient) {
  app.get('/api/sfu/:roomId/capabilities', { preHandler: [app.authenticate] }, async request => {
  ...
  app.post('/api/sfu/:roomId/transport', { preHandler: [app.authenticate] }, async request => {
    ...
    const transport = await router.createWebRtcTransport({
      listenInfos: [{ protocol: 'udp', ip: listenIp, announcedAddress: announcedIp }, { protocol: 'tcp', ip: listenIp, announcedAddress: announcedIp }],
      enableUdp: true,
      enableTcp: true,
      preferUdp: true,
  ...
  app.post('/api/sfu/transport/:id/connect', { preHandler: [app.authenticate] }, async request => {
    const transport = transports.get(String((request.params as any).id));
  ...
  app.get('/api/sfu/:roomId/producers', { preHandler: [app.authenticate] }, async request => {
    ...
    return { roomId, producerIds: [...producers.values()].filter(p => !p.closed && p.rtpParameters.encodings?.length).map(p => p.id), canConsume: (producerId: string) => router.canConsume({ producerId, rtpCapabilities: router.rtpCapabilities }) };
  });
  ...
  app.post('/api/sfu/transport/:id/consume', { preHandler: [app.authenticate] }, async request => {
    ...
    consumer.on('transportclose', () => { void consumer.close(); });
  ...
  app.post('/api/sfu/consumer/:id/resume', { preHandler: [app.authenticate] }, async request => ({ ok: true, consumerId: String((request.params as any).id) }));

  app.get('/api/sfu/health', async () => ({ ok: Boolean(worker && !worker.closed), workers: worker && !worker.closed ? 1 : 0, rooms: [...routers.keys()], transports: transports.size, producers: producers.size, rtcPortRange: `${process.env.MEDIASOUP_RTC_MIN_PORT ?? 40000}-${process.env.MEDIASOUP_RTC_MAX_PORT ?? 49999}` }));
}
