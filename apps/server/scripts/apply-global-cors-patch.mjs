...
if (start === -1 || end === -1) { console.error('Global CORS patch: isAllowedOrigin was not found.'); process.exit(1); }
const replacement = `const isAllowedOrigin = (origin?: string | null) => {
  ...
  const isNativeApp = /^capacitor:\\/\\/localhost$/.test(origin) || /^ionic:\\/\\/localhost$/.test(origin);
...
if (corsStart === -1 || corsEnd === -1) { console.error('Global CORS patch: Fastify CORS registration was not found.'); process.exit(1); }
...
source = source.slice(0, corsStart) + corsReplacement + source.slice(corsEnd + 4);
