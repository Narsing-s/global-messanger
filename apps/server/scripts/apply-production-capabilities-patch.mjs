...
if (fs.existsSync(capability)) {
  ...
  // global-market-platform already owns these public API-key and webhook creation routes.
  // Keep the richer production-capability routes, but remove duplicate Fastify registrations.
  s = s.replace(/\n  \/\/ P3 — executable bot runtime, mini-app manifests, developer API keys, webhooks, business inbox\/catalog\.[\s\S]*?\n  app\.post\('\/api\/developer\/bots'/, "\n  // P3 — executable bot runtime, mini-app manifests, business inbox/catalog.\n  app.post('/api/developer/bots'");
